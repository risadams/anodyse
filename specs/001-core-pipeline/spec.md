...
- Q: When running `anodyse .` on a directory, what should be included in index.md? → A: Follow includes — if a playbook includes a role, document that role separately and add it to the index
...
An operations team has 20 playbooks and roles across a directory, with some playbooks referencing roles. They want a single command to generate docs for all items (including referenced roles) and produce an `index.md` that lists all items, their descriptions, and tags for easy navigation.
...
**Independent Test**: Can run `anodyse . --output ./docs` from a directory containing multiple playbooks/roles and receive both individual .md files and an index.md listing all items and their referenced roles.
...
2. **Given** a playbook that includes roles, **When** processed, **Then** the referenced roles are documented separately and appear in the index.md alongside the playbook.

3. **Given** playbooks with `@tag` annotations, **When** the index is generated, **Then** each item in the index includes its tags for categorization.

4. **Given** an existing docs directory with output files, **When** Anodyse runs again, **Then** existing .md files are backed up with `.bak` extension before being overwritten (unless `--no-backup` flag is used).
...
- What happens if a playbook includes the same role multiple times?
- What happens if role discovery creates circular dependencies (role A includes role B which includes role A)?
...
- **FR-009**: When processing a directory, parser MUST follow role includes and document referenced roles separately (semantic role discovery).
...
- **FR-014**: Output handler MUST generate an `index.md` listing all parsed items (including role dependencies discovered via role includes) with their `@tag` values and one-line descriptions (mandatory output).
